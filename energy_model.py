...
class EnergyMobileNet(LightningModule):
    ...
    def __init__(
        ...
        self.ce_loss_module = nn.CrossEntropyLoss()
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss, EC_out, EC_in = 0, 0, 0
        if indom_idxs.sum() > 0:
            loss += self.ce_loss_module(logits[indom_idxs], labels[indom_idxs])
            EC_in = -torch.logsumexp(logits[indom_idxs], dim=1)
            loss += 0.1 * torch.pow(F.relu(EC_in - self.m_in), 2).mean()
        if ood_idxs.sum() > 0:
            EC_out = -torch.logsumexp(logits[ood_idxs], dim=1)
            loss += 0.1 * torch.pow(F.relu(self.m_out - EC_out), 2).mean()
